 ...
 */
package org.xbib.elasticsearch.plugin.bundle.index.analysis.worddelimitergraph;
 ...
 * the deprecated {@link org.xbib.elasticsearch.plugin.bundle.index.analysis.worddelimiter.WordDelimiterFilter},
 * this token filter produces a correct token graph as output.  However, it cannot consume an input token
 ...
 * </ul>
 * One use for {@link WordDelimiterGraphFilter2} is to help match words with different
...
public final class WordDelimiterGraphFilter2 extends TokenFilter {
     ...
     */
    public static final int ALL_PARTS_AT_SAME_POSITION = 1024;
     ...
     */
    public WordDelimiterGraphFilter2(TokenStream in, byte[] charTypeTable, int configurationFlags, CharArraySet protWords) {
        super(in);
     ...
     * @param protWords If not null is the set of tokens to protect from being delimited
     */
    public WordDelimiterGraphFilter2(TokenStream in, int configurationFlags, CharArraySet protWords) {
    ...
    private void bufferWordParts() throws IOException {
        ...
        // if length by start + end offsets doesn't match the term's text then set offsets for all our word parts/concats
        // to the incoming offsets.  this can happen if WDGF is applied to an injected synonym, or to a stem'd form, etc:
    ...
    public boolean incrementToken() throws IOException {
        while (true) {
            ...
            if (bufferedPos < bufferedLen) {
                ...
                posIncAttribute.setPositionIncrement(has(ALL_PARTS_AT_SAME_POSITION) ?
                        accumPosInc : accumPosInc + startPos - wordPos);
                accumPosInc = 0;
    ...
    /**
     * startPos, endPos -{@literal >} graph start/end position
     * startPart, endPart -{@literal >} slice of the original term for this part
     */
